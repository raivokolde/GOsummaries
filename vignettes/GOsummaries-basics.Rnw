...
For such cases there is a parameter \texttt{wc\_data} in \texttt{gosummaries.default} where one can enter arbitrary data that will be shown on word clouds. The input structure is similar to the gene list input , only instead of vectors with gene names it requires data frames with two columns: "Term" and "Score". Where Term is the text that is being drawn and Score determines its size. 
...
One can also add the gene lists when specifying \texttt{wc\_data}, but they can be in many cases omitted. This option makes it easy to incorporate the GO enrichment results from other tools. 

\subsection{Attributes}
Several general properties of the plot are stored in the attributes of \texttt{gosummaries} object. If needed, these can be changed using the \texttt{attr} function. 

\begin{itemize}
	\item \texttt{score\_type}: Specifies how to handle the scores associated to words in word clouds. In case of "count", the score is expected to be positive and the word sizes are directly proportional to the scores. In case of "p-value", the word sizes are proportional to $-\log_{10}$ of the score. 
	\item \texttt{wc\_algorithm} Specifies the word cloud layout algorithm. In case of "top", the word placement starts from the top corner, in case of "middle" from the centre of left or right side of the box. 
	\item \texttt{wordcloud\_legend\_title} Gives the title of the word cloud.
\end{itemize}

\subsection{Displaying gene names instead of GO categories}
If the gene lists or the whole dataset is very small then the GO analysis might not give many significant results. In these cases it would be more reasonable to show the names of genes instead. It is possible to add the gene lists as described above with \texttt{wc\_data}. However, more convenient means are implemented for PCA, limma and MDS results in \texttt{gosummaries.prcomp}, \texttt{gosummaries.MArrayLM} and \texttt{gosummaries.matrix} respectively. 

In each case there is a parameter \texttt{show\_genes} that toggles if gene names or GO categories are shown. The parameters to decide the importance for different genes vary between these functions. For PCA we use the size of the component loadings, for limma we use the adjusted p-value and for MDS we use the p-values of Spearman rank correlation with the components. 

As gene identifiers in expression matrices can be unintelligible then by default these functions convert the identifiers into gene names using \texttt{gconvert} function from \texttt{gProfileR} package. It is possible to turn this function off as well by setting parameter \texttt{gconvert\_target} to \texttt{NULL}.  

These options are especially important when use \texttt{GOsummaries} on data that is not describing genes. For example, PCA analysis is often used for other high throughput experiments, such as metabolomics and metagenomics. Using the \texttt{GOsummaries} approach on these datasets can be very revealing. 

\subsubsection{Example: metagenomics}
Principal Coordinate Analysis is very common on metagenomics data. To conveniently visualise these results with \texttt{GOsummaries}, there is a function \texttt{gosummaries.matrix}. Since the rows represent taxa instead of genes we cannot use GO enrichment analysis, but we can show the names of taxons. 
...
> gs = gosummaries(pcoa, metagenomic_example$otu, metagenomic_example$annot, show_genes = T, gconvert_target = NULL, n_genes = 30)
> 
...
\begin{figure}[ht]
	\centering
	\includegraphics[width=140mm]{metagenomics.pdf}
